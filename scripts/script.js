...
function createPin(pinData, index) {
  ...
  const selectedPicLink = `selectedpic.html?imageName=${pinData.imageName}&content=${pinData.content}&userProfileImageName=${pinData.userProfileImageName}&link=${pinData.link}&alt=${pinData.alt}&pinId=${pinId}`;
...
//   pinsData[pinIndex].userProfileImageName = updatedPin.UserProfileImageName;
//   pinsData[pinIndex].content = updatedPin.content;
//   pinsData[pinIndex].link = updatedPin.Link;

//   // Update the displayed content on the page
//   updatePinContent(pinIndex);
// }

// // Function to update the displayed pin content
// function updatePinContent(pinIndex) {
//   const pinId = `pin${pinIndex + 1}`;
//   const pinElement = document.getElementById(pinId);

//   // Update the title content
//   pinElement.querySelector('.user').alt = pinsData[pinIndex].userProfileImageName;

//   // Update the content
//   pinElement.querySelector('.content').textContent = pinsData[pinIndex].content;

//   // Update the link
//   pinElement.querySelector('.bottom-bar a').textContent = `↗ ${pinsData[pinIndex].link}`;
// }

// // Loop through the pins data and create each pin
// // pinsData.forEach((pinData, index) => createPin(pinData, index));

// // Add click event listeners to the created pins
// document.querySelectorAll('.window').forEach((pinElement, pinIndex) => {
//   pinElement.addEventListener('click', function () {
//       const selectedPicLink = `selectedpic.html?imageName=${pinsData[pinIndex].imageName}&content=${pinsData[pinIndex].content}&userProfileImageName=${pinsData[pinIndex].userProfileImageName}&link=${pinsData[pinIndex].link}&alt=${pinsData[pinIndex].alt}&pinId=${pinElement.id}`;
//       window.location.href = selectedPicLink; // Redirect to the selectedpic.html page
//   });
//   pinElement.querySelector('.hover-zone .top-bar p').addEventListener('click', function () {
//       editPin(pinIndex);
//   });
// });
